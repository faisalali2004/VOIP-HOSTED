    ...
    while True:
        try:
            ...
            if message == 'exit':
                ...
                break
            elif message in user_sessions and user_sessions[message]['status'] == 'online':
                conn.send(f'Request sent to {message}. Waiting for response...'.encode('utf-8'))
                target_conn = user_sessions[message]['connection']
                target_conn.send(f'{username} wants to communicate. Type "yes" to accept or "no" to decline: '.encode('utf-8'))
                    ...
                    def relay_data(source, target):
                        while True:
                            try:
                                ...
                                if data.decode('utf-8', errors='ignore').strip() == 'exit':
